...
import type { RoomStatusEvent, RoomStatusChangedPayload } from '../types';
...
export class RoomStatusController {
  ...
  constructor(io: Server) {
    ...
    if (PrismaClient && process.env.DATABASE_URL) {
      try { this.prisma = new PrismaClient(); } catch { this.prisma = null; }
    }
  }

  async setStatus(roomNumber: string, status: RoomStatusEvent, meta?: Record<string, any>) {
    const rec: RoomRecord = { number: roomNumber, status };
  ...
  private mapStatusToRoomUpdate(status: RoomStatusEvent) {
